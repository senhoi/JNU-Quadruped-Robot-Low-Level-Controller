...
static void R1dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan);
static void R2dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan);
static void R3dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan);
static void R4dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan);
static void WriteDataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan);
...
static uint8_t canTransmit(SCA_Handler_t *pSCA, uint8_t *TxBuf, uint8_t TxLen)
{
	...
	while (pSCA->Can->Send(pSCA->ID, TxBuf, TxLen) && (waitime < pSCA->Can->Retry))
		waitime++;
...
static void R2dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan)
{
	...
	/* 第二类读写命令为IQ8格式 */
	temp = ((int16_t)hcan->pRxMsg->Data[1]) << 8;
	temp |= ((int16_t)hcan->pRxMsg->Data[2]) << 0;

	/* 在第二类读写命令中，电压数据为IQ10格式 */
	if (hcan->pRxMsg->Data[0] == R2_Voltage)
...
static void R3dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan)
{
	...
	/* 第三类读写命令以IQ24格式传输 */
	temp = ((int32_t)hcan->pRxMsg->Data[1]) << 24;
	temp |= ((int32_t)hcan->pRxMsg->Data[2]) << 16;
	temp |= ((int32_t)hcan->pRxMsg->Data[3]) << 8;
	temp |= ((int32_t)hcan->pRxMsg->Data[4]) << 0;

	/* 速度和电流使用标值，需要将转换值乘以该参数的最大值得到实际值 */
	if ((hcan->pRxMsg->Data[0] == R3_Velocity) || (hcan->pRxMsg->Data[0] == R3_VelocityLimit))
		RxData = (float)temp / IQ24 * Velocity_Max;

	else if ((hcan->pRxMsg->Data[0] == R3_Current) || (hcan->pRxMsg->Data[0] == R3_CurrentLimit))
		RxData = (float)temp / IQ24 * pSCA->Current_Max;

	else if (hcan->pRxMsg->Data[0] == R3_BlockEngy)
...
static void R4dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan)
{
	...
	temp = ((int32_t)hcan->pRxMsg->Data[4]) << 24;
	temp |= ((int32_t)hcan->pRxMsg->Data[5]) << 16;
	pSCA->Velocity_Real = (float)temp / IQ30 * Velocity_Max;

	temp = ((int32_t)hcan->pRxMsg->Data[6]) << 24;
	temp |= ((int32_t)hcan->pRxMsg->Data[7]) << 16;
...
/**
  ...
  * @返	回	无
  */
static void R5dataProcess(SCA_Handler_t *pSCA, CAN_HandleTypeDef* hcan)
{
	/* 装填序列号 */
	pSCA->Serial_Num[0] = hcan->pRxMsg->Data[1];
	pSCA->Serial_Num[1] = hcan->pRxMsg->Data[2];
	pSCA->Serial_Num[2] = hcan->pRxMsg->Data[3];
	pSCA->Serial_Num[3] = hcan->pRxMsg->Data[4];
...
{
	/* 写入成功，将缓存中的参数更新到句柄中 */
	if (hcan->pRxMsg->Data[1] == Actr_Enable)
	{
		/* 有新数据写入成功，复位存储标志位 */
...
/**
  ...
  * @返	回	无
  */
void canDispatch(CAN_HandleTypeDef* hcan)
{
	SCA_Handler_t *pSCA = getInstance((uint8_t)hcan->pRxMsg->StdId);
	...
	case R3_HomingCurrentLimitH:
	case R3_BlockEngy:
		R3dataProcess(pSCA, hcan);
		break;

	case R4_CVP:
		R4dataProcess(pSCA, hcan);
		break;

	case R5_ShakeHands:
		R5dataProcess(pSCA, hcan);
		break;

	/* 其余为写入指令，判断写入是否成功，更新句柄 */
	default:
		WriteDataProcess(pSCA, hcan);
		break;
	}
}
